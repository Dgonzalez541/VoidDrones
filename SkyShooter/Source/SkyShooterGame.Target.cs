...
public class SkyShooterGameTarget : TargetRules
{
	public SkyShooterGameTarget(TargetInfo Target) : base(Target)
	{
	...
	internal static void ApplySharedLyraTargetSettings(TargetRules Target)
    {
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			SkyShooterGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
        {
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("SkyShooterGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}		
	}
